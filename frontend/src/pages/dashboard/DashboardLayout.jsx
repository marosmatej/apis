...
import { useGetUsersQuery } from '../../redux/AdminApi';

import  UserDashboard from './users/UserDashboard';
import ManageBooks from './manageBooks/ManageBooks';
import EditBookModal from './manageBooks/EditBookModal';

const DashboardLayout = () => {
  
   const navigate = useNavigate();
  const { data: users = [], isLoading, isError } = useGetUsersQuery();
  const [modalState, setModalState] = useState({ isOpen: false, mode: "", book: null });
  ...
  const handleModalClose = () => {
    setModalState({ isOpen: false, mode: "", book: null });
  };

  const handleModalSave = (updatedBook) => {
    // Refresh books or update state
    handleModalClose();
  };
      ...
      <main className="p-6 sm:p-10 space-y-6 ">
        <div className="flex flex-col space-y-6 md:space-y-0 md:flex-row justify-between">
          ...
          <div className="flex flex-col md:flex-row items-start justify-end -mb-3">
            <Link
              to="#"
              onClick={() => setModalState({ isOpen: true, mode: "create", book: null })}
